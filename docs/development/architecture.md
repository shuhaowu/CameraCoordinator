...
- **`CameraCoordinator`** (`camera_coordinator.go`) — aggregates one or more `CameraDetector` instances into a single event stream. Tracks active detectors per device and deduplicates: emits one `RecordingOn` when the first detector goes active for a device, and one `RecordingOff` when the last detector goes inactive. Fans out deduplicated events to N output channels (one per notifier), closing them when `Run` returns.
...
Events flow from detectors through the coordinator and out to notifiers. Each box represents a logical component, not a specific variable or goroutine.

```mermaid
flowchart LR
    D1["Detector A"]
    D2["Detector B"]
    Coordinator["<b>Coordinator</b><br/>stateful merge, dedup & fan‑out"]
    A1["Notifier 1"]
    A2["Notifier 2"]
    A3["Notifier 3"]

    D1 & D2 --> Coordinator --> A1
    Coordinator --> A2
    Coordinator --> A3
```
